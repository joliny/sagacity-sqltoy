...
public class SqlServerDialectUtils {
		...
		if (!entityMeta.getCascadeModels().isEmpty()) {
			...
			for (TableCascadeModel cascadeModel : entityMeta.getCascadeModels()) {
				...
				if (subTableData != null && !subTableData.isEmpty()) {
					...
					saveAll(sqlToyContext, subTableData, new AbstractReflectPropertyHandler() {
						@Override
                        public void process() {
	...
	private static Long saveAll(SqlToyContext sqlToyContext, EntityMeta entityMeta, PKStrategy pkStrategy,
			boolean isAssignPK, String insertSql, List<?> entities, AbstractReflectPropertyHandler reflectPropertyHandler,
	 ...
	 * @throws Exception
	 */
	private static Long batchUpdateByJdbc(AbstractTypeHandler typeHandler, final String updateSql,
			...
			for (TableCascadeModel cascadeModel : entityMeta.getCascadeModels()) {
				...
				if (subTableData != null && !subTableData.isEmpty()) {
					...
					saveOrUpdateAll(sqlToyContext, subTableData, sqlToyContext.getBatchSize(),
							...
							new AbstractReflectPropertyHandler() {
								@Override
                                public void process() {
									for (int i = 0; i < mappedFields.length; i++) {
										this.setValue(mappedFields[i], mainFieldValues[i]);
